...
// const gaugeCtx = document.getElementById('temperature-gauge').getContext('2d');
...
// Helper function to determine color index
...
function getColorForValue(celsius) {
    if (celsius < 20) return '#4ec66b';  // Green
    if (celsius < 30) return '#006de8'; // Blue
    return '#c0971a';                   // Red
}
function getColorForLocalValue(celsius) {
  if (celsius < 100) return '#4ec66b';  // Green
  if (celsius < 150) return '#006de8';  // Blue
  return '#c0971a';                     // Red
}
...
function getColorForPresure(pressure) {
    if (pressure < 800) return '#4ec66b';  // Green
    if (pressure < 1000) return '#006de8';  // Blue
    return '#c0971a';                     // Red
}

function getColorForHumidity(humidity){
     if (humidity < 30) return '#4ec66b';  // Green
    if (humidity < 60) return '#006de8';  // Blue
    return '#c0971a';     
}
const gaugeChartLocalTemp = new Chart(temperatureLocalGuaugeCtx, {
  ...
  plugins: [{
    ...
    afterDatasetDraw(chart) {
      ...
      // Get the current temperature from the correct element
      ...
      // Calculate percentage based on your gauge range (0-100)
      const percent = Math.min(Math.max(temperature / 100, 0), 1);
      const angle = Math.PI * percent - Math.PI;  // Needle rotation from left (π) to right (0)

      const cx = width / 2;
      const cy = height * 0.82; // bottom center
...
function updateGauge(temperature, humidity, pressure, temperature_locale) {
  ...
  // const gaugeValueDiv = document.getElementById("temperature-gaugeValue");
  // if (gaugeValueDiv) {
  //   gaugeValueDiv.textContent = `${temperature} °C`;
  // }
...
function updateBoxes(temperature, humidity, pressure, altitude, state) {
  ...
  if (state === "INIT" || state === "REINIT" || state === "ENDTREATMENT" || state === "TREATMENT" ) {
...
const temperatureChart = createLineChart(temperatureHistoryCtx, 'Temperature', MIN_TEMP, MAX_TEMP, getColorForValue);
const temperatureLocalChart = createLineChart(temperatureLocalCtx, 'Temperature Local',MIN_TEMP, MAX_TEMP, getColorForLocalValue);
const presurehistoryChart = createLineChart(presureCtx, "Presure", 500, 1500, getColorForPresure )
const humidityHistoryChart = createLineChart(humidityCtx,"Humidity", 0, 100, getColorForHumidity )
// Data Management
const chartRanges = {
  temperature: { min: 0, max: 100, buffer: 10, unit: '°C', colorFn: getColorForValue },
  temperature_locale: { min: 0, max: 100, buffer: 10, unit: '°C', colorFn: getColorForLocalValue },
  pressure: { min: 800, max: 1100, buffer: 100, unit: 'hPa', colorFn: getColorForPresure },
  humidity: { min: 0, max: 100, buffer: 10, unit: '%', colorFn: getColorForHumidity }
};

function updateLineChart(chart, value, chartType) {
  const range = chartRanges[chartType];
  ...
  chart.options.scales.y.min = Math.min(numValue - range.buffer, range.min);
  chart.options.scales.y.max = Math.max(numValue + range.buffer, range.max);
  
  // Last point colored, others transparent
  chart.data.datasets[0].pointBackgroundColor = 
    chart.data.datasets[0].data.map((v, i, arr) => 
      i === arr.length - 1 ? range.colorFn(v) : 'transparent');
  
  // Update the display
  const viewDiv = document.getElementById(`${chartType}-history-view`);
  if (viewDiv) {
    viewDiv.textContent = `${numValue.toFixed(2)} ${range.unit}`;
  }
...
temperatureChart.data.datasets[0].data = new Array(9).fill(null);
temperatureChart.update();
function updateSensorReadings() {
    fetch(`http://localhost:3001/sensorReadings`)
   ...
   updateLineChart(temperatureChart, temperature, 'temperature');
      updateLineChart(temperatureLocalChart, temperature_locale, 'temperature_locale');
      updateLineChart(presurehistoryChart, pressure, 'pressure');
      updateLineChart(humidityHistoryChart, humidity, 'humidity');
